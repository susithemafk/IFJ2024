...
void compareToknes(FILE *expectedFile, LinkedList *buffer) {
    ...
    for (unsigned int i = 0; i < size; i++) {
        ...
        bool result = (strcmp(token->value, expected_token) == 0);
        testCase(
            test,
            result,
...
int main(void) {
    ...
    FILE *input_file = fopen("./test_inputs/lexical/input.txt", "r");

    enum ERR_CODES err = firstPass(table, input_file, buffer);
    printf("first pass err: %d\n", err);
    FILE *expected_file = fopen("./test_inputs/lexical/expected_output.txt", "r");
