...
#  2024-09-25  Upraveny identifikatory rozsireni pro IFJ24
#  2024-10-25  Upraven vyraz pro kontrolu loginu (podpora delky 8 i 9 znaku) v souboru rozdeleni
...
touch $LOG
ARCHIVE=`basename $1`
NAME=`echo $ARCHIVE | cut -d . -f 1 | egrep "^x[a-z]{5,6}[0-9][0-9a-z]$"`
  ...
  declare -a LOGINS
  {
    ...
    while read -a RADEK; do
      if [[ "${RADEK[0]}" != "" ]]; then
        ...
        LOGINCHECK=`echo ${RADEK[0]} | egrep "x([a-z]{5,6}[0-9][0-9a-z]|[0-9]{6})$"`
...
if [[ -f rozsireni ]]; then
  ...
  RETCODE=$?
  if [[ $RETCODE = "0" ]]; then
    UNKNOWN=`cat rozsireni | grep -v -E -e "^(ORELSE|UNREACHABLE|BOOLTHEN|FOR|WHILE|FUNEXP)$" | wc -l`
    if [[ $UNKNOWN = "0" ]]; then
      echo_color green "OK" 
    else
